...
enum PlaidAPIError: LocalizedError {
    ...
    /// ⚠️ 这些是**会显示给用户**的文案，必须走 String.loc() ——
    ...
    var errorDescription: String? {
        switch self {
        case .notSignedIn:
            return String.loc("尚未登录，请先用 Apple ID 登录")
        case .unauthorized:
            return String.loc("登录已过期，请重新登录")
        case .plaid(_, let message):
            return message ?? String.loc("银行数据接口暂时不可用，请稍后重试")
        case .server(let status, let message):
            return message ?? String.loc("服务器响应异常（状态码：\(status)）")
        case .transport(let error):
            return String.loc("网络请求失败：\(error.localizedDescription)")
        case .decoding(let error):
            return String.loc("数据解析失败：\(error.localizedDescription)")
        }
    }
